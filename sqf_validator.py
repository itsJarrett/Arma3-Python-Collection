...
def check_sqf_syntax(filepath):
    ...
    with io.open(filepath, 'r', encoding='utf-8', errors='ignore') as file:
        content = file.read()
                ...
                if (isInCommentBlock == False):
                    if (ignoreTillEndOfLine): # we are in a line comment, just continue going through the characters until we find an end of line
                        ...
                        if (c == '"' or c == "'"):
                            ...
                            if (len(if_list) > 0):
                                #if (if_list[-1] in ['f']):
                                    #print("ERROR: Possible missing white space after if detected at {0} Line number: {1}".format(filepath,lineNumber))
                                    #bad_count_file += 1
                                if_list.append('(')
                        elif (c == ')'):
